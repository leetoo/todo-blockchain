...
case class SimpleState(
  ...
  override def changes(block: SimpleBlock): Try[BoxStateChanges[PublicKey25519Proposition, SimpleBox]] =
    Try {
      val initial = (Seq(): Seq[Array[Byte]], Seq(): Seq[SimpleBox])

      val (toRemove: Seq[ADKey], toAdd: Seq[SimpleBox]) =
      ...
      val ops: Seq[BoxStateChangeOperation[PublicKey25519Proposition, SimpleBox]] =
        toAdd.map(b => Insertion[PublicKey25519Proposition, SimpleBox](b)) ++
          Seq(Insertion[PublicKey25519Proposition, SimpleBox](forgerBox))
